...
EXPERIMENT_NAME = "example_addstrict_noisymemory"
RANDOM_SEED = 0
N_SAMPLES = 500     # WHEN EXTRACTING RESULTS, CHANGE 2000 to 500 IN THE IPYNB FILE
SUCCESS_L2_THRESHOLD = 2.5  # 0.0 means no add, 99999.0 means always add
DELETION_L2_THRESHOLD = 99999.0  # 99999.0 means no deletion
DELETION_RETREIVAL_N_TIMES = 99999  # 99999 means no deletion
CLEANUP_RETRIEVAL_N_TIMES = 0  # 0 means no cleanup
EXIST_TIME_THRESHOLD = 99999  # 99999 means no cleanup
MIN_MEMORY_SIZE = 1
...
## Experiment: add a l2=5.0 noise to all mem experiences, which is located at:
# memory_agent.experience_memory.values is a list of {"token": token, ..., "ego_fut_traj": experience_traj}
for memory in memory_agent.experience_memory.values:
    experience_traj = memory["ego_fut_traj"][1:]
    noise = np.random.normal(0, 5.0 / np.sqrt(2), experience_traj.shape)
    memory["ego_fut_traj"][1:] = experience_traj + noise
